...
const getPortalDashboard = async (req, res) => {
  if (req.user.role === "student") {
    const student = await Student.findOne({ user: req.user._id }).populate("classRoom");
    if (!student) return res.status(404).json({ message: "Student profile not found." });

    const [assignments, attendance, results, materials, fees, notices] = await Promise.all([
      ...
      FeeRecord.find({ student: student._id }).sort({ dueDate: 1 }).limit(6),
      Notification.find({ $or: [{ audience: "all" }, { audience: "students" }] }).sort({ createdAt: -1 }).limit(6),
    ]);

    return res.json({ role: "student", profile: student, assignments, attendance, results, materials, fees, notices });
  }
  ...
  if (req.user.role === "parent") {
    ...
    const [attendance, results, fees, notices] = await Promise.all([
      ...
      FeeRecord.find({ student: { $in: childIds } }).sort({ dueDate: 1 }).limit(8).populate("student", "rollNumber"),
      Notification.find({ $or: [{ audience: "all" }, { audience: "parents" }] }).sort({ createdAt: -1 }).limit(6),
    ]);

    return res.json({ role: "parent", profile: parent, attendance, results, fees, notices });
  }
